...
public class AccountFlowsService implements IAccountFlowsService {
		...
		try {
			...
			if(null!=ret ){
				 if(Integer.valueOf(ret[0])==0){
					 ...
					 String sql = "update T_ACCOUNTFLOWS t set t.status='1' or t.t.status='3'  where t.TRANSACTIONSEQNUM='"
							 + contractTranstion.getTranstNo() + "'";
	...
	public void updateAccountFlowById(String transactionSeqNum) {
		String sql="update T_ACCOUNTFLOWS t set t.status='1' where t.TRANSACTIONSEQNUM='"+transactionSeqNum+"'";
		 accountFlowsDao.createSQLQuery(sql).executeUpdate();
	}
...
//			}
	...
	@Override
	public void saveAccountFlows(AccountFlows accountFlows) {
		accountFlowsDao.save(accountFlows);
	}

	@Override
	public void updateAccountFlowSattus(String id) {
		String sql="update T_ACCOUNTFLOWS t set t.status='0' where t.id='"+id+"'";
		 accountFlowsDao.createSQLQuery(sql).executeUpdate();
	}
}