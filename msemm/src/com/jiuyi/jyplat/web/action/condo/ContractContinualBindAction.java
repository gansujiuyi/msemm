				...
				if(contractMoney.subtract(needPayMoney).doubleValue() >= -0.00001 && contractMoney.subtract(needPayMoney).doubleValue() <= 0.00001){
					...
					for (int d = 0; d < contr_.length; d++) {
						...
						for (int e = 0; e < contrA_.length;e++) {
							...
							condoPayInfo.setPayBankName("民生银行兰州分行");
...
//							contactInfo_.setSuperviseAmt(String.valueOf(isNeedMoney));
	...
	@AuthName
	@Action(value = "/saveAccountFlows", results = { @Result(name = SUCCESS, type = "json", params = {
			"includeProperties", "message" }) })
	public String saveAccountFlows() {
		try {
			flows.setStatus("2");
			flows.setDebitCreditType("0001");
	...
	@AuthName
	@Action(value = "/checkAccountFlows", results = { @Result(name = SUCCESS, type = "json", params = {
			"includeProperties", "message" }) })
	public String checkAccountFlows() {
		try {
			AccountFlows temp = accountFlowsService.getAccountFlowsBySeqNum(flows);
			if(null==temp){
				message = "ok";
			}else{
				message = "no";
    ...
    public AccountFlows getFlows() {
		return flows;
	}

	public void setFlows(AccountFlows flows) {
		this.flows = flows;
	}
